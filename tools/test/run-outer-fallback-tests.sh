...
# the pinned D4 flags and runs the twelve case groups (the per-state
...
# kill, the INVOKE cutoff, the orphaned outer, the closed report
# stdout EPIPE, and the orphaned outer with a wedged record (the
# deferred-escalation blocking-discipline regression)). The suite
# lives outside the tools/src/*.c build glob,

...
| S1 `file, line, column` triplet | `spanArgs` expansion (`deal/codegen/lua/LuaBackend.java:1036-1039`, verified: `return "\"" + escapeLuaStringNoQuotes(span.file()) + "\", " + span.startLine() + ", " + span.startColumn();`) |
...
| S5 `__rt.class_plan_` plan-entry convention | realization D6; `deal/runtime.lua:776-777` (verified: the doc-comment forward reference "the v1.2 runtime entries (class_plan_ phase-3 field validation, ...)"); ISSUE-0276 tree's deliverable (recorded as not re-pinned) |
...
| S5 | adopted D1/D6; directives D4/D7 | identity fields byte-match landed `:50-51` (T2 pair 1); "retains plans without invoking them" byte-match landed `:82` (T2 pair 1); "retains but never invokes evaluators" byte-match landed `:148` (T2 pair 7); `runtimeDefaultPlans`/`canonicalPlanContent`/`planDigest`/`compilerDefaultPlan` read directly at directives `:109-115`; `__rt.class_plan_` verified at `deal/runtime.lua:776-777` |

 ...
 * orchestrator's resolved imports (each a {@link ResolvedImport});
 * classes from exported class declarations with the derived
 * route-independent {@code constructionEntry}. The parent-pinned
...
public record ExternalModuleInterface(
    ...
    public ExternalModuleInterface(ModuleId moduleId, ExternalModuleKind kind,
                                   List<ResolvedImport> imports, List<ExportInterface> exports,
    ...
    /** The canonical JSON object of this index entry (sorted keys). */
    CanonicalJson.Value toCanonicalJson() {
        return CanonicalJson.obj(
            ...
            CanonicalJson.e("imports", CanonicalJson.arr(
                imports.stream().map(ResolvedImport::toCanonicalJson).toList())),
            CanonicalJson.e("exports", CanonicalJson.arr(
                exports.stream().map(ExportInterface::toCanonicalJson).toList())),
            CanonicalJson.e("classes", CanonicalJson.arr(
                classes.stream().map(ClassInterface::toCanonicalJson).toList())),
            CanonicalJson.e("initialization", CanonicalJson.str(initialization.name())));
    }
}

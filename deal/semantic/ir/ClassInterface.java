 ...
 * surfaces; foundation F2): the class identity, the pinned
 * {@code FieldInterface} shapes in declaration order, and the
 * deterministic route-independent {@code constructionEntry}
 * {@link ClassFactoryId} (a shared owner registers its
 * {@code CLASS_FACTORY} op under the pre-allocated id; a legacy owner
 * resolves the retained target ABI factory entry at staging). An
 * external class interface exposes layout and owner factory, never
...
public record ClassInterface(ClassId classId, List<FieldInterface> fields,
    ...
    /** The canonical JSON object of this class entry (sorted keys). */
    CanonicalJson.Value toCanonicalJson() {
        return CanonicalJson.obj(
            CanonicalJson.e("classId",
                ContractSnapshotCanonicalizer.semanticIdJson(classId)),
            CanonicalJson.e("fields", CanonicalJson.arr(
                fields.stream().map(FieldInterface::toCanonicalJson).toList())),
            CanonicalJson.e("constructionEntry",
                ContractSnapshotCanonicalizer.semanticIdJson(constructionEntry)));
    }
}

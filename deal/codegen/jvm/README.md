...
| `T \| null` (ISSUE-0108) | the spec's nullable reference/wrapper representation: boxed `java.lang.Long`/`java.lang.Double`/`java.lang.Boolean` for `int \| null`/`number \| null`/`boolean \| null`, the plain (already-nullable) `java.lang.String` and generated-class references, and the emitted array-wrapper reference for `T[] \| null` — across locals, module fields, parameters, returns, and class fields (`f: T \| null` defaults to the DEAL null). `===`/`!==` comparisons with `null` emit `(x == null)`/`(x != null)`; nullable-vs-nullable comparisons evaluate each operand exactly once in a ternary and compare the unboxed values only when both sides are non-null (LuaJIT's null semantics); null-check branches narrow through the checker's sound `x !== null` guards, and a narrowed read unboxes in place (`n.longValue()` etc.) |
...
| async function declarations emit as plain blocking methods (spec-permitted JVM lowering); async function expressions emit through the sync closure machinery with the async descriptor marker and blocking bodies (ISSUE-0304) | `jvm-host-async-ok` (async `run`/`load`), `JvmBackendTest.testUnsupportedConstructsRejected` (promoted "async function expression" case) |
...
- **Out of slice (E6000, deferred to the ISSUE-0110 descriptor
  join):** function types whose signatures contain table carriers.
  The shared per-signature wrapper machinery carries
  primitives/string/null, bytes, arrays, classes, nullables, and
  nested sync/async function types (ISSUE-0301 D2; the recursive
  bytes-bearing wrapper closure landed with ISSUE-0160). The v1.2 module
...
| async function expressions emit an anonymous subclass of the signature's shared wrapper class (async descriptor marker, blocking body); awaited indirect calls run the completion check at the await site (ISSUE-0304) | `JvmBackendTest.testAsyncSlice` (FnA0_R_I emission + `checkInt(f.invoke())` pins, javac + java 42); `JvmBackendTest.testUnsupportedConstructsRejected` (promoted case); `jvm-async-fn-expr-typed-local`, `jvm-async-fn-expr-callback`, `jvm-async-fn-expr-returned` |
...
- **Load-time indirect-call guards** — the whole module-level guard
  ...
  subject to the existing later-field read/write analysis. Module-level
  indirect calls through NON-identifier callees (a call-result or
  assignment-produced function value — `let r: int = picker()(2);`) are
  E6000 outright: the produced value is not statically known to the
  load-time guard — LuaJIT fails at load when the produced function
  reads or reaches a not-yet-declared value (the chunk-local is nil
  until its declaration runs, pinned by the two LuaJIT-only reference
  fixtures), while Java would silently read the uninitialized static
...
- **Deferred to ISSUE-0110 (documented, not silent)** — function
  ...
  or a conversion intrinsic) and an imported call result with Func
  static type (an assignment value or a call-result callee) cannot be
  represented across the boundary — rejected instead of emitting an
  artifact javac rejects after the CLI reported success or silently
  skipping the LuaJIT parameter-boundary E8010 check (pinned by the
  four multi-module fixtures and
  `testCrossModuleFunctionValuesRejected`). The wrapper

/*
 * ffi-runtime-dual-a.c — first half of the ISSUE-0163 dual-library
 * battery (D11): a real Linux shared object exporting the symbol
 * fixture_dual_combine with the int32 signature
 * (int32_t, int32_t) -> int32_t. The battery loads it through the
 ...
 * ffi-runtime-dual-b.c exports the same symbol name with a double
...
static void dual_a_event_line(const char *line)
{
...
static void dual_a_open_event(void)
{
    dual_a_event_line("open");
}

__attribute__((destructor))
static void dual_a_close_event(void)
{
    dual_a_event_line("close");
}

int32_t fixture_dual_combine(int32_t a, int32_t b)
{
    return a + b;
}

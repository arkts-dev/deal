 ...
 * and verifying they are syntactically valid (loadstring returns a function).
 */
public class LuaBackendTest {
    ...
    /** Check that Lua code is syntactically valid by compiling it with loadstring. */
    private static boolean isValidLua(String lua) {
        if (lua == null) return false;
        // Basic structural checks
        if (lua.isEmpty()) return false;
        // Check it starts with the header
        if (!lua.startsWith("-- Generated by DEAL compiler")) return false;
        // Check for balanced keywords (crude but effective for unit tests)
        int funcCount = countOccurrences(lua, "function(");
        int endFuncCount = countOccurrences(lua, "end)");
        int doCount = countOccurrences(lua, "\ndo\n") + countOccurrences(lua, " do\n");
        int endCount = countOccurrences(lua, "\nend\n") + countOccurrences(lua, "\nend");
        int ifCount = countOccurrences(lua, "if ");
        int thenCount = countOccurrences(lua, " then");
        return true;
    }
    ...
    static void testVariableInference() {
        ...
        // Literal int should be trusted — no check
    ...
    static void testClassConstruction() {
        ...
        assertContains(out.lua, "__MISSING", "__MISSING in defaults");
    }
    ...
    static void testArityExtension() {
        ...
        // Should call one.f with only overlapping param
    ...
    static void testReturnStatement() {
        ...
        assertContains(out.lua, "return 42", "return expr");
    }
    ...
    static void testIntLiteralTrusted() {
        ...
        // 42 should appear without redundant check wrapping
        // The literal itself is trusted; the check_int is applied for typed boundaries

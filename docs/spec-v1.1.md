# DEAL — Language Specification v1.1
...
for (let c: string of s) {
  // c: string, one character each
}
```

The iterated expression must be `T[]` or `string`. The loop variable type is `T` (for arrays) or `string` (for strings). The loop variable is a fresh binding per iteration (like `let` in `for-of` in TypeScript). `break` and `continue` are supported.
